...
/*
	...
	Description:
	Used in the vehicle Container menu, stores the selected item and puts it in the vehicles virtual inventory
...
_totalWeight = [life_cont_vehicle] call life_fnc_vehicleWeight;
_itemWeight = ([_ctrl] call life_fnc_itemWeight) * _num;
_veh_data = life_cont_vehicle getVariable ["Container",[[],0]];
...
if(_ctrl == "money") then
{
	...
	life_cont_vehicle setVariable["Container",[_inv,(_veh_data select 1) + _itemWeight],true];
	[life_cont_vehicle] call life_fnc_vehInventory;
}
	else
{
